...
- `dock_back_to_sleep.yaml`: waits for a return after an overnight absence, applies a signed target-temperature adjustment, and restores the original setpoint. Its default warming step approximates Sleepme's documented +5 °F behavior in Celsius-oriented installations and remains user-configurable.
